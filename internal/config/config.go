...
type Config struct {
	...
	// admin-only reload endpoint. APIKeyAgent is a separate, least-
	// privilege credential for Certificate Agents: it can only fetch a
	// certificate bundle (cert + private key), never list inventory or
	// trigger a reload. Leaving all three unset disables the API entirely
	// (fails closed) rather than serving it unauthenticated.
...
func Load() {
	...
	viper.SetDefault("API_KEY_AGENT", "")
	...
	App = Config{
		...
		APIKeyAgent:        viper.GetString("API_KEY_AGENT"),
